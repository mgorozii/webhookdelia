...
import (
	...
	"github.com/mgorozii/webhookdelia/internal/models"
...
func (s *service) Send(record models.Record, message string, parseMode tb.ParseMode) error {
	if parseMode == "" {
		parseMode = tb.ModeMarkdown
	}

	if _, err := s.bot.Send(record, message, parseMode); err != nil {
...
func (s *service) onStart(m *tb.Message) {
	...
	if record, err := s.store.GetByChatID(m.Chat.ID); err == nil {
		if err := s.Send(record, s.formatMessage(record), tb.ModeHTML); err != nil {
			s.log.Error(
				"can't send message",
